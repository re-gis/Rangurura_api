const IdeaSchema=require("../../models/idea.model"); 
...
const IdeaController=async(req,res)=>{
    ...
    if(!category || !igitekerezo || !urwego){
        return    res.status(400).json({message:"Vuga igitekerezo cyawe  byibuze ushyireho urwego nakategori yacyo ubundi wohereze!"});
       }
    try{
        ...
        await newIdea.save()
return res.status(200).json({message:"Urakoze gutanga umusanzu wo kubaka igihugu cyawe!"});
}catch(error){
    console.log(error);
    res.status().json({error:"Ntabwo igitekerezo cyawe cyoherejwe ongera ugerageze mukanya"});
}
}
